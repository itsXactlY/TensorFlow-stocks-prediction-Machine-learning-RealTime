...
startdate = "2024-01-01" #"2024-07-22"
enddate = "2024-08-08" #"2024-08-08"
'''
Bullrun      2021-02-01 2022-08-31
...
Downtrend1       2023-07-21 2023-08-23
Downtrend2       2021-04-14 2022-03-24
BTC STARTS AT 2017, 8, 17
'''
timeframe = "1m"
symbol = "BTC" + "USDT_klines"
start_date = dt.datetime.strptime(startdate, "%Y-%m-%d")
end_date = dt.datetime.strptime(enddate, "%Y-%m-%d")

# Fetch data
df = pd.DataFrame(MSSQLData.get_data_from_db(connection_string, symbol, timeframe, start_date, end_date))
if df.empty:
    raise ValueError("No data returned from the database. Please check your query and date range.")
...
# aLca :: i dont want an loop, we just run once for now.
...
df_raw = df #pd.read_csv(file_rwa_alpha, parse_dates=['Date'], index_col='Date', sep='\t')
for c in df_raw.columns:
    df_raw = df_raw.rename(columns={c: c.lower()})
df_raw.index = pd.to_datetime(df_raw.index, errors="coerce")
print("\n[1] Read data ohlcv data from MSSQL", ' Shape_raw: ', df_raw.shape)

# Resample the data into x intervals
df_resampled = df.resample(ind).agg({
    'Open': 'first',   # Take the first open in the 4-hour window
...
print("Resampled DataFrame:", df_resampled.head())

# Replace original df with the resampled dataframe
df = df_resampled
print(df.__len__())
print("\n[1.1] Resampled MSSQL into proper Timeframe: ", ind)
